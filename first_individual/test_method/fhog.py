...
def getFeatureMaps(image, k, mapp):
	...
	r, alfa = func1(dx, dy, boundary_x, boundary_y, height, width, numChannels) #func1 without @jit(nopython=True)  ###
	...
	'''
	### 200x speedup
	r, alfa = func1(dx, dy, boundary_x, boundary_y, height, width, numChannels) #with @jit(nopython=True)
	### ~0.001s
	...
	'''
	### original implementation
	mapp['map'] = func2(dx, dy, boundary_x, boundary_y, r, alfa, nearest, w, k, height, width, sizeX, sizeY, p, stringSize) #func2 without @jit(nopython=True)  ###
	'''
	### 500x speedup
	mapp['map'] = func2(dx, dy, boundary_x, boundary_y, r, alfa, nearest, w, k, height, width, sizeX, sizeY, p, stringSize) #with @jit(nopython=True)
	### ~0.001s
...
def normalizeAndTruncate(mapp, alfa):
	...
	'''
	### original implementation
	newData = func3(partOfNorm, mapp['map'], sizeX, sizeY, p, xp, pp) #func3 without @jit(nopython=True)  ###
	...
	'''
	### 30x speedup
	newData = func3(partOfNorm, mapp['map'], sizeX, sizeY, p, xp, pp) #with @jit(nopython=True)
	###
...
def PCAFeatureMaps(mapp):
	...
	'''
	### original implementation
	newData = func4(mapp['map'], p, sizeX, sizeY, pp, yp, xp, nx, ny) #func without @jit(nopython=True)  ###
	...
	'''
	### 190x speedup
	newData = func4(mapp['map'], p, sizeX, sizeY, pp, yp, xp, nx, ny) #with @jit(nopython=True)
	###
